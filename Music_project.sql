...
/* Q1: Who are the top 5 senior most employee based on job title? */
SELECT * FROM employee 
ORDER BY levels DESC
LIMIT 5;
/* Q2: Which countries has the most Invoices? */
SELECT billing_country,COUNT(billing_country) AS COUNT_OF_INVOICES 
FROM invoice
...
SELECT 
	...
	SUM(total) as total_number_of_invoices_from_each_city
	FROM invoice 
GROUP BY billing_city
ORDER BY total_number_of_invoices_from_each_city DESC
...
ORDER BY email 
/* With out using Sub Quary*/
SELECT distinct c.email, c.first_name, c.last_name FROM 
customer c
JOIN invoice i ON c.customer_id=i.customer_id
JOIN invoice_line i_l ON i.invoice_id=i_l.invoice_id
Join track t1 on i_l.track_id=t1.track_id
...
WHERE track.genre_id IN (SELECT genre_id 
	FROM genre 
